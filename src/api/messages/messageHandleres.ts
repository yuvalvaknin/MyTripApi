...
export const getAllMessageBetweenTwoUsers = async (req: Request, res: Response) => {
    try {
        const users: getAllMessageBetweenTwoUsersDto = req.body;
        const messages: Message[] = await MessageModel.find({fromUser: users.firstUser, toUser: users.secondUser});
        messages.push(...await MessageModel.find({fromUser: users.secondUser, toUser: users.firstUser}));
        res.json(messages);
    } catch (error) {
