...
const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    jwt.verify(token, process.env.JWT_REFRESH_TOKEN, async (err : any, jwtUser : any) => {
        ...
        try {
            ...
            const accessToken = jwt.sign({ _id : verifiedUser._id, userName : verifiedUser.userName } as UserJWTPaylod, process.env.JWT_ACCESS_TOKEN || '', { expiresIn: process.env.JWT_EXPIRATION });
            const refreshToken = jwt.sign({ _id: verifiedUser._id, userName : verifiedUser.userName } as UserJWTPaylod, process.env.JWT_REFRESH_TOKEN || '');
            
            res.cookie('access_token', accessToken);
            res.cookie('refresh_token', refreshToken);

            verifiedUser.tokens = verifiedUser.tokens.map(tok => tok === token ? refreshToken : tok );
            await verifiedUser.save();
